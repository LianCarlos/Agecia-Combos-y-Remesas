...
/* ─── SVG Icons (smaller for bottom nav) ─── */

function DashIcon({ active }: { active: boolean }) {
  return (
    <svg className={`h-5 w-5 ${active ? 'text-brand-green' : 'text-slate-400'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
...
export function MobileBottomNav({ isSuperAdmin }: MobileBottomNavProps) {
  const pathname = usePathname();

  const navItems = [
    { href: '/admin', icon: DashIcon, label: 'Inicio' },
    { href: '/admin/currencies', icon: GlobeIcon, label: 'Países' },
    { href: '/admin/payment-methods', icon: PayIcon, label: 'Pagos' },
    { href: '/admin/exchange-rates', icon: ChartIcon, label: 'Tasas' },
    { href: '/admin/combos', icon: GiftIcon, label: 'Combos' },
    { href: '/admin/recargas', icon: PhoneIcon, label: 'Recargas' },
    ...(isSuperAdmin
      ? [{ href: '/admin/employees', icon: UsersIcon, label: 'Empleados' }]
      : []),
  ];
